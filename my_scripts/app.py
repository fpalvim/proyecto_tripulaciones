...
def update_event_database_render(current_events_df):
    ...
    try:
        ...
        columns_str = ', '.join([f"{col} TEXT" for col in columns_list])
        ...
        if new_events_to_insert:
            insert_placeholders = ', '.join(['%s'] * (len(columns_list) + 1))
...
if all_events_df_render is not None:
    ...
    print(all_events_df_render.head())